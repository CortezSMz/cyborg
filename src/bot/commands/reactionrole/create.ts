...
export default class ReactionRoleCreateCommand extends Command {
	constructor() {
		super('reactionrole-create', {
			description: {
				content: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.REACTIONROLE.CREATE.DESCRIPTION,
			},
			channel: 'guild',
			...
			args: [
				{
					id: 'title',
					prompt: {
						start: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.REACTIONROLE.CREATE.PROMPT.START_TITLE(message.author),
					},
				},
				{
					id: 'color',
	...
	public async exec(message: Message, { title, color }: { title: string; color: string }) {
		const embed = new MessageEmbed().setTitle(title).setColor(color || COLORS.EMBED);
