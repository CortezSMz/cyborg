...
export default class SetConfigMemberLogCommand extends Command {
	public constructor() {
		super('config-set-autorole', {
			description: {
				content: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.CONFIG.SET.AUTO_ROLE.DESCRIPTION.CONTENT,
				usage: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.CONFIG.SET.AUTO_ROLE.DESCRIPTION.USAGE,
				examples: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.CONFIG.SET.AUTO_ROLE.DESCRIPTION.EXAMPLES,
			},
			category: 'config',
			channel: 'guild',
