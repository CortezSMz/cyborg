...
export default class ToggleRoleStateCommand extends Command {
	public constructor() {
		super('config-toggle-role-state', {
			description: {
				content: (message: Message) => this.client.LOCALE(message.guild!).COMMANDS.CONFIG.TOGGLE.ROLE_STATE.DESCRIPTION.CONTENT,
				usage: () => null,
				examples: () => null,
			},
			category: 'config',
	...
	public async exec(message: Message) {
		...
		if (roleState) {
			...
			return message.util?.reply(this.client.LOCALE(message.guild!).COMMANDS.CONFIG.TOGGLE.ROLE_STATE.REPLY_DEACTIVATED);
		}
		...
		return message.util?.reply(this.client.LOCALE(message.guild!).COMMANDS.CONFIG.TOGGLE.ROLE_STATE.REPLY_ACTIVATED);
	}
}
