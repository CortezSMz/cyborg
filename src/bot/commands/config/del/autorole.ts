...
export default class DeleteConfigMemberLogCommand extends Command {
	public constructor() {
		super('config-del-autorole', {
			description: {
				content: (message: Message) => LOCALE(message.guild!).COMMANDS.CONFIG.DELETE.AUTO_ROLE.DESCRIPTION.CONTENT,
				usage: () => null,
				examples: () => null,
			},
			category: 'config',
	...
	public async exec(message: Message) {
		...
		return message.util?.reply(LOCALE(message.guild!).COMMANDS.CONFIG.DELETE.AUTO_ROLE.REPLY);
	}
}
