	...
	COMMANDS: {
		...
		CONFIG: {
			DESCRIPTION: stripIndents`Available methods:
				...
				Available keys:
				• logs \`<channel>\` - Keep track of all messages on the server.
				• memberlog \`<channel>\` - Keep track of who joins/leaves the server.

				Toggle keys:
				 • rolestate - Keep track and preserve roles on joins/leaves.
			...
			SET: {
				...
				GUILD_LOG: {
					...
					PROMPT: {
						START: (author: User | null) => `${author}, what channel should I send the messages?`,
					},
		...
		BAN: {
			...
			MESSAGE: (guild: string, reason?: string) => stripIndents`
				**You have been banned from ${guild}**
				${reason ? `\n**Reason:** ${reason}\n` : ''}
				You can appeal your ban by DMing \`Corteez#0002\` with a message why you think you deserve to have your ban lifted.
			`,
