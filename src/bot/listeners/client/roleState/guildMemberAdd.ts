...
export default class GuildMemberAddRoleStateListener extends Listener {
	...
	public async exec(member: GuildMember) {
		...
		const autorole = this.client.settings.get(member.guild, SETTINGS.AUTO_ROLE);

		if (autorole) {
			try {
				await member.roles.add(autorole, 'Automatic role');
			} catch (error) {
		...
		if (roleState) {
			...
			try {
				if (user) {
					await member.roles.add(user.roles, 'Automatic role');
				}
			} catch (error) {
				this.client.logger.error(error);
			}
