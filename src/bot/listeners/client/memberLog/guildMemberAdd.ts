...
export default class GuildMemberAddMemberLogListener extends Listener {
	...
	public async exec(member: GuildMember) {
		...
		if (memberlog) {
			const embed = new MessageEmbed().setColor(COLORS.MEMBER_JOIN).setAuthor(`${member.user.tag} (${member.id})`, member.user.displayAvatarURL()).setFooter('User joined').setTimestamp(new Date());
