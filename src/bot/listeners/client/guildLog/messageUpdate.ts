	...
	public async exec(oldMessage: Message, newMessage: Message) {
		...
		if (guildLogs) {
			...
			const embed = new MessageEmbed()
				...
				.addField('ﾅ Channel', newMessage.channel);
			let msg = '';
			if (/```(.*?)```/s.test(oldMessage.content) && /```(.*?)```/s.test(newMessage.content)) {
				...
				const prepend = '```diff\n';
				const append = '\n```';
				embed.addField('ﾅ Message', `${prepend}${msg.substring(0, 1000)}${append}`);
			} else {
				...
				embed.addField('ﾅ Message', `${msg.substring(0, 1020)}` || '\u200b');
			}
			embed
				.addField('ﾅ Message', `[Jump To](${newMessage.url})`, true)
